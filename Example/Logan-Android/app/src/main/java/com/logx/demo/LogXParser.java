...
public class LogXParser {
    ...
    public LogXParser(byte[] encryptKey16, byte[] encryptIv16) {
    ...
    public void parse(InputStream is, OutputStream os) {
        ...
        try {
            ...
            for (int i = 0; i < content.length; i++) {
                ...
                if (start == '\1') {
                    i++;
                    int length = (content[i] & 0xFF) << 24 | (content[i + 1] & 0xFF) << 16 | (content[i + 2] & 0xFF)
                            << 8 | (content[i + 3] & 0xFF);
                    i += 3;
                    int type;
                    if (length > 0) {
                        ...
                        try {
                            ...
                            inflaterOs = new GZIPInputStream(new CipherInputStream(new ByteArrayInputStream(dest),
                                    mDecryptCipher));
                            int e = 0;
