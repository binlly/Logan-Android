...
int init_zlib_clogx(cLogx_model *model) {
    int ret = 1;
    if (model->zlib_type == LOGX_ZLIB_INIT) { //如果是init的状态则不需要init
        return Z_OK;
    }
    ...
    if (NULL != temp_zlib) {
        ...
        if (ret == Z_OK) {
            ...
            model->zlib_type = LOGX_ZLIB_INIT;
        } else {
            model->is_ready_gzip = 0;
            model->zlib_type = LOGX_ZLIB_FAIL;
        }
    } else {
...
void clogx_zlib(cLogx_model *model, char *data, int data_len, int type) {
    int is_gzip = model->is_ready_gzip;
    int ret;
    if (is_gzip) {
        unsigned int have;
        unsigned char out[LOGX_CHUNK];
        z_stream *strm = model->strm;

        strm->avail_in = (uInt) data_len;
        strm->next_in = (unsigned char *) data;
        do {
            strm->avail_out = LOGX_CHUNK;
            ...
            if (Z_STREAM_ERROR == ret) {
                ...
                model->is_ready_gzip = 0;
                model->zlib_type = LOGX_ZLIB_END;
            } else {
                have = LOGX_CHUNK - strm->avail_out;
                ...
                if (handler_len) {
                    ...
                    aes_encrypt_clogx((unsigned char *) gzip_data, model->last_point, handler_len,
...
void clogx_zlib_end_compress(cLogx_model *model) {
    clogx_zlib(model, NULL, 0, Z_FINISH);
    ...
    aes_encrypt_clogx((unsigned char *) data, model->last_point, 16,
                       (unsigned char *) model->aes_iv); //把加密数据写入缓存
    model->last_point += 16;
    *(model->last_point) = LOGX_WRITE_PROTOCOL_TAIL;
    model->last_point++;
...
void clogx_zlib_compress(cLogx_model *model, char *data, int data_len) {
    if (model->zlib_type == LOGX_ZLIB_ING || model->zlib_type == LOGX_ZLIB_INIT) {
        model->zlib_type = LOGX_ZLIB_ING;
        clogx_zlib(model, data, data_len, Z_SYNC_FLUSH);
    } else {
        init_zlib_clogx(model);
    }
}

void clogx_zlib_delete_stream(cLogx_model *model) {
    (void) deflateEnd(model->strm);
    model->zlib_type = LOGX_ZLIB_END;
    model->is_ready_gzip = 0;
}
