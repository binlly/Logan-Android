...
construct_json_data_clogx(char *log, int flag, long long local_time, char *thread_name,
    ...
    Construct_Data_cLogx *construct_data = NULL;
    cJSON *root = NULL;
    ...
    if (NULL != root) {
        if (NULL != map) {
            add_item_string_clogx(map, log_key, log);
            add_item_number_clogx(map, flag_key, (double) flag);
            add_item_number_clogx(map, localtime_key, (double) local_time);
            add_item_string_clogx(map, threadname_key, thread_name);
            add_item_number_clogx(map, threadid_key, (double) thread_id);
            add_item_bool_clogx(map, ismain_key, is_main);
            inflate_json_by_map_clogx(root, map);
            char *back_data = cJSON_PrintUnformatted(root);
            construct_data = (Construct_Data_cLogx *) malloc(sizeof(Construct_Data_cLogx));

            if (NULL != construct_data) {
                memset(construct_data, 0, sizeof(Construct_Data_cLogx));
                ...
                if (NULL != temp_data) {
                    ...
                    construct_data = NULL;
                    printf_clogx(
                            "construct_json_data_clogx > malloc memory fail for temp_data\n");
                }
            }
            free(back_data);
        }
...
void construct_data_delete_clogx(Construct_Data_cLogx *item) {
    if (NULL != item) {
        if (NULL != item->data) {
            free(item->data);
        }
