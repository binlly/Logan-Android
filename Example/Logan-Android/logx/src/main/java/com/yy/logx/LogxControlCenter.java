...
class LogxControlCenter {

    private static LogxControlCenter sLogxControlCenter;

    private ConcurrentLinkedQueue<LogxModel> mCacheLogQueue = new ConcurrentLinkedQueue<>();
    ...
    private LogxThread mLogxThread;
    private SimpleDateFormat dataFormat = new SimpleDateFormat("yyyy-MM-dd");

    private LogxControlCenter(LogxConfig config) {
        if (!config.isValid()) {
    ...
    private void init() {
        if (mLogxThread == null) {
            mLogxThread = new LogxThread(mCacheLogQueue, mCachePath, mPath, mSaveTime,
                    mMaxLogFile, mMinSDCard, mEncryptKey16, mEncryptIv16);
            mLogxThread.start();
        }
    }

    static LogxControlCenter instance(LogxConfig config) {
        if (sLogxControlCenter == null) {
            synchronized (LogxControlCenter.class) {
                if (sLogxControlCenter == null) {
                    sLogxControlCenter = new LogxControlCenter(config);
                }
            }
        }
        return sLogxControlCenter;
    }
    ...
    void write(String log, int flag) {
        ...
        model.action = LogxModel.Action.WRITE;
    ...
    void flush() {
        ...
        LogxModel model = new LogxModel();
        model.action = LogxModel.Action.FLUSH;
        mCacheLogQueue.add(model);
        if (mLogxThread != null) {
            mLogxThread.notifyRun();
        }
