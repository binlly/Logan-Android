...
#include "clogx_protocol.h"

JNIEXPORT jint JNICALL
Java_com_yy_logx_CLogxProtocol_clogx_1write(JNIEnv *env, jobject instance, jint flag,
    ...
    jint code = (jint) clogx_write(flag, log, local_time, thread_name, thread_id, is_main);
...
JNIEXPORT jint JNICALL
Java_com_yy_logx_CLogxProtocol_clogx_1init(JNIEnv *env, jobject instance,
                                                    jstring cache_path_,
    ...
    jint code = (jint) clogx_init(cache_path, dir_path, max_file, encrypt_key16, encrypt_iv16);
...
JNIEXPORT jint JNICALL
Java_com_yy_logx_CLogxProtocol_clogx_1open(JNIEnv *env, jobject instance,
                                                    jstring file_name_) {
    const char *file_name = (*env)->GetStringUTFChars(env, file_name_, 0);

    jint code = (jint) clogx_open(file_name);
...
Java_com_yy_logx_CLogxProtocol_clogx_1flush(JNIEnv *env, jobject instance) {
    clogx_flush();
}

JNIEXPORT void JNICALL
Java_com_yy_logx_CLogxProtocol_clogx_1debug(JNIEnv *env, jobject instance,
                                                     jboolean is_debug) {
    int i = 1;
    if (!is_debug) {
        i = 0;
